...
\begin{figure}[h]
  ...
  \node[rectangleTX, below right= 1cm of s4](s7) {Dienst 7\\Verbrauchsdaten};
  \node[rectangleTX, below left= 1cm of s4](s5) {Dienst 5\\Buchungsabonnement};
  \node[rectangleTX, right= 1cm of s6](s8) {Dienst 8\\Fahrzeugfreischaltung};
  %   arrows
...
\section{Dienst 8 -- Fahrzeugfreischaltung (unlock)}
Dienst 8 dient dem freischalten von Fahrzeugen bzgl. Buchungszielen durch das RIS. Dies erlaubt die Öffnung bzw. Freischaltung das Fahrzeug innerhalb der Endandwenderanwendung des RIS, z.B. innerhalb einer mobilen App.

\subsection*{Funktionen}
\begin{itemize}
\item Freischaltung Fahrzeug
\end{itemize}
