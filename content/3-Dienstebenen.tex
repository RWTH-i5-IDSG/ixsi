...
Um eine Kopplung zwischen FVS und RIS zu realisieren, muss mindestens der Dienst 1 Statische Daten (static data) von beiden Interaktionspartnern unterstützt werden. In \cref{fig:depend} sind die Abhängigkeiten zwiscchen den Diensten dargestellt.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
  % Dienste
  \node[circleTX](s1) {Dienst 1};
  \node[circleTX, below left of=s1](s3) {Dienst 3};
  \node[circleTX, left of=s3](s2) {Dienst 2};
  \node[circleTX, below right of=s1](s4) {Dienst 4};
  \node[circleTX, right of=s4](s6) {Dienst 6};
  \node[circleTX, below of=s4](s5) {Dienst 5};
...
Dienst 1 dient dem Austausch von Inforomationen über Anbieter und statischen Daten von Buchungszielen. Hierzu gehören Anbieter, Standort- und Fahrzeugdaten.
...
Die tatsächliche Verfügbarkeitszeiten von über Dienst 1 ausgetauschen Buchungszielen wird während der Reiseauskunft durch das RIS beim FVS abgerufen. 
